%===============================================================
%   Design Documentation
%===============================================================

\documentclass[parskip=full]{scrartcl}

\usepackage{enumerate}
\usepackage[shortlabels]{enumitem}
\usepackage[utf8]{inputenc} % use utf8 file encoding for TeX sources
\usepackage[T1]{fontenc} % avoid garbled Unicode text in pdf
\usepackage{hyperref} % detailed hyperlink/pdf configuration
\usepackage{csquotes} % provides \enquote{} macro for "quotes"
\usepackage{graphicx} % embed graphics
\usepackage[toc]{glossaries}
\usepackage{pdfpages}
\usepackage{wrapfig}

% definitions
\newcommand{\pseProjectName}{An Interactive End-To-End Machine Learning Platform}
\newcommand{\pseTitle}{\pseProjectName}
\newcommand{\pseDocTitle}{Design}

\hypersetup{ % ‘texdoc hyperref‘ for options
pdftitle={\pseDocTitle}}
\title{%
    \pseTitle \\
    \large \pseDocTitle
    \date{\today}
}
\makenoidxglossaries
\loadglsentries{glossary}
\begin{document}

\begin{titlepage}
    \maketitle
    \begin{center}
        \begin{tabular}{ r l }
            Requirements Specification & \textbf{Murat Kurnaz}         \\
            Design                     & \textbf{Mustafa Enes Batur}   \\
            Implementation             & \textbf{Ömer Erdinç Yağmurlu} \\
            QA / Testing               & \textbf{Tarek Gaddour}        \\
            Final                      & \textbf{Atalay Donat}         \\
        \end{tabular}
    \end{center}
\end{titlepage}

\tableofcontents

\include{sections/introduction}
\newpage
\include{sections/structure}
\newpage
\include{sections/api}
\newpage
\include{sections/class-index}
\newpage
\include{sections/class-description}
\newpage
\include{sections/processes}
\newpage
\include{sections/appendix}

\includepdf[pages=1-8]{rasterbations/1.pdf}
\includepdf[pages=1-4]{rasterbations/2.pdf}
\includepdf[pages=1-8]{rasterbations/3.pdf}
\includepdf[pages=1-8]{rasterbations/4.pdf}
\includepdf[pages=1-8]{rasterbations/5.pdf}

\printnoidxglossary

% END DOCUMENT
\end{document}
